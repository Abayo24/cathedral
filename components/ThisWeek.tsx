				...
				<div className='grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-8'>
					<div>
						<div className='flex items-center justify-between mb-4'>
							...
							<Link href='/events' className='font-ui text-sm text-royal hover:text-crimson inline-flex items-center gap-1.5 no-underline'>
					...
					<div className='space-y-5'>
						<div className='bg-white rounded-3xl border border-parchment/70 p-6'>
								...
								<ul className='divide-y divide-parchment/70'>
									{announcements.slice(0, 3).map((a) => (
										<li key={a.id} className='py-3 first:pt-0 last:pb-0'>
											<p className='font-ui font-semibold text-navy-mid flex items-start gap-1.5'>
											...
											<p className='font-ui text-xs text-faint mt-1'>{formatShortDate(a.publishedAt)}</p>
										</li>
									))}
								</ul>
							) : (
							...
							<Link href='/bulletins' className='mt-4 font-ui text-sm text-royal hover:text-crimson inline-flex items-center gap-1.5 no-underline'>
						...
						{latestBulletin && (
							<div className='bg-royal rounded-3xl p-6 text-white'>
								...
								<p className='font-ui text-sm text-white/80 mt-1'>{formatDayKey(latestBulletin.date, 'long')}</p>
